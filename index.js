// ## Array Cardio Day 2

const people = [
  { name: "Wes", year: 1988 },
  { name: "Kait", year: 1986 },
  { name: "Irv", year: 1970 },
  { name: "Lux", year: 2015 },
];

const comments = [
  { text: "Love this!", id: 523423 },
  { text: "Super good", id: 823423 },
  { text: "You are the best", id: 2039842 },
  { text: "Ramen is my fav food ever", id: 123523 },
  { text: "Nice Nice Nice!", id: 542328 },
];

// Some and Every Checks
// some tests whether at least one element in an array passes the test implemented by the provided callback function. It returns a boolean value (true or false) and does not modify the original array

// Array.prototype.some() // is at least one person 19 or older?
const isAdult = people.some((p) => new Date().getFullYear() - p.year >= 19);
console.log({ isAdult });
// Array.prototype.every() // is everyone 19 or older?

const isAllAdult = people.every((p) => new Date().getFullYear() - p.year >= 19);
console.log({ isAllAdult });

// Array.prototype.find()
// Find is like filter, but instead returns just the one you are looking for
// find the comment with the ID of 823423
const findComment = comments.find((comment) => comment.id === 823423);
console.log({ findComment });

// Array.prototype.findIndex()\
const findIndexComment = comments.findIndex((comment) => comment.id === 823423);
console.log(findIndexComment);

// Find the comment with this ID
const findCommentId = comments.findIndex((comment) => comment.id === 823423);
console.log({ findCommentId });
// delete the comment with the ID of 823423

const deleteComment = comments.filter((comment) => comment.id !== 823423);

console.table(deleteComment);
